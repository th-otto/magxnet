...
#else
#define PPP_CHANNELS	1				/* number of PPP units */
#endif
...
static long cdecl pppdev_open(MX_DOSFD *f);
static long cdecl pppdev_close(MX_DOSFD *);
static long cdecl pppdev_write(MX_DOSFD *, const char *, long);
static long cdecl pppdev_read(MX_DOSFD *, char *, long);
static long cdecl pppdev_lseek(MX_DOSFD *, long, short);
static long cdecl pppdev_stat(MX_DOSFD *f, MAGX_UNSEL *unsel, short rwflag, long /* APPL * */ appl);
static long cdecl pppdev_ioctl(MX_DOSFD *, short, void *);
static long cdecl pppdev_datime(MX_DOSFD *, short *, short);
static long cdecl pppdev_delete(MX_DOSFD *f, MX_DOSDIR *dir);
...
MX_DDEV cdecl_ppp_dev GNU_ASM_NAME("cdecl_ppp_dev") = {
	pppdev_open,
	pppdev_close,
	/* FIXME: arguments swapped */
	(long cdecl (*)(MX_DOSFD *, long, void *))pppdev_read,
	(long cdecl (*)(MX_DOSFD *, long, void *))pppdev_write,
	pppdev_stat,
	pppdev_lseek,
	pppdev_datime,
	pppdev_ioctl,
...
extern MX_DDEV ppp_dev GNU_ASM_NAME("ppp_dev");
...
static ushort ppp_fcs(const char *cp, long len)
{
	ushort fcs = FCS_INIT;
...
static long ppp_link(struct iflink *ifl)
{
	struct ppp *ppp;
	short i, j;
...
static long cdecl ppp_open(struct netif *nif)
{
...
static long cdecl ppp_close(struct netif *nif)
{
	struct ppp *ppp = nif->data;
	long r = 0;
...
static long cdecl ppp_output(struct netif *nif, BUF *buf, const char *hwaddr, short hwlen, short pktype)
{
	struct ppp *ppp = nif->data;
...
static long cdecl ppp_ioctl(struct netif *nif, short cmd, long arg)
{
	struct iflink *ifl;
	struct ifreq *ifr;
	struct ppp *ppp = nif->data;
	...
	case SIOCGIFNAME:
		...
		if (!(ppp->flags & PPPF_LINKED))
		{
			DEBUG(("ppp_ioctl: IFLINK: chan %d not linked", nif->mtu));
			return EINVAL;
		}
	...
	case SIOCGLNKFLAGS:
		...
		ifr->ifr_flags = (short) ppp->opts;
		return 0;

	case SIOCSLNKFLAGS:
		ifr = (struct ifreq *) arg;
		ppp->opts = (ulong) ifr->ifr_flags;
		return 0;

	case SIOCGLNKSTATS:
		ifr = (struct ifreq *) arg;
		l = (long *)ifr->ifr_data;
		memset(l, 0, 40 * sizeof(long));
...
long ppp_init(void)
{
	...
	sprintf(devname, "PPP  v%s, %d channels\n\r", PPP_VERSION, PPP_CHANNELS);
	c_conws(devname);
	for (i = 0; i < PPP_CHANNELS; ++i)
	{
...
#ifdef __PUREC__
		/* FIXME: use Dcntl() binding */
		{
		long _gemdos(void, ...);
		_gemdos(0x130, DEV_M_INSTALL, devname, (long) &ppp_dev);
		}
#else
		Dcntl(DEV_M_INSTALL, devname, (long) &ppp_dev);
#endif
	}
	...
	while (nbytes > 0)
	{
		...
		while (space > 0 && nbytes > 0)
		{
			...
			while (--nbytes >= 0)
			{
				...
				case FLAG:
					b = ppp->rcv_buf;
					if (ppp->esc || cp - b->dstart < 3 || ppp->flags & PPPF_DROP)
					{
						long l = cp - b->dstart;
...
static short ppp_send(struct slbuf *slb)
{
#define PPP_PUTC(_c) { buf[head++] = _c; head &= mask; }

	struct ppp *ppp = slb->nif->data;
	short head;
	short mask = slb->osize - 1;
	long space;
	long nbytes;
	unsigned char c;
	char *cp;
...
static long ppp_send_frame(struct ppp *ppp, BUF *buf, short proto)
{
	BUF *nbuf;
	long r;
	...
	if ((r = if_enqueue(&ppp->nif->snd, nbuf, nbuf->info)) != 0)
	{
		ppp->stat.o_qfull++;
		ppp->nif->out_errors++;
 ...
 */
static void ppp_build_frame(struct ppp *ppp, BUF *buf)
{
	ushort fcs;
	ushort proto;
	unsigned char type;
	 ...
	 */
	if (ppp->opts & PPPO_COMPRESS && proto == PPPPROTO_IP && IP_PROTO(buf) == IPPROTO_TCP)
	{
...
static void wakeup(PROC *p, long arg)
{
	int i;
...
static void ppp_recv_usr(struct ppp *ppp, BUF *buf)
{
	if (ppp->links <= 0)
	{
...
static void ppp_recv_frame(struct ppp *ppp, BUF *b)
{
	ushort proto;
	unsigned char *cp;
	long len;
	...
	case PPPPROTO_UNCOMP_TCP:
		if (ppp->opts & PPPO_COMPRESS)
			r = slc_uncompress(b, TYPE_UNCOMPRESSED_TCP, ppp->comp);
		else if ((ppp->opts & PPPO_AUTOCOMP) && (r = slc_uncompress(b, TYPE_UNCOMPRESSED_TCP, ppp->comp)) != 0)
	...
	if (proto == PPPPROTO_IP && ppp->nif->flags & IFF_UP && !(ppp->opts & PPPO_IP_DOWN))
	{
		if (ppp->nif->bpf)
		{
			b->dstart -= 4;
...
static long cdecl pppdev_open(MX_DOSFD *fp)
{
	struct ppp *ppp = &ppp_priv[0];
...
static long cdecl pppdev_write(MX_DOSFD *fp, const char *_buf, long nbytes)
{
	struct ppp *ppp = &ppp_priv[0];
...
static long cdecl pppdev_read(MX_DOSFD *fp, char *buf, long nbytes)
{
	struct ppp *ppp = &ppp_priv[0];
	long cando;
	BUF *b;
...
static long cdecl pppdev_lseek(MX_DOSFD *fp, long where, short whence)
{
	UNUSED(fp);
	UNUSED(where);
	UNUSED(whence);
...
static long cdecl pppdev_ioctl(MX_DOSFD *fp, short cmd, void *arg)
{
	struct ppp *ppp = &ppp_priv[0];
	long *l;
	BUF *b;
	...
	case FIONWRITE:
		*(long *) arg = ppp->nif->snd.qlen < ppp->nif->snd.maxqlen ? 1 : 0;
		return 0;
...
static long cdecl pppdev_datime(MX_DOSFD *fp, short *timeptr, short mode)
{
	UNUSED(fp);
	if (mode == 0)
	{
...
static long cdecl pppdev_close(MX_DOSFD *fp)
{
	struct ppp *ppp = &ppp_priv[0];

	if (fp->fd_refcnt <= 0 && --ppp->links <= 0)
	{
		ppp->pgrp = 0;
		if_flushq(&ppp->rcvq);
	}
...
static long cdecl pppdev_delete(MX_DOSFD *fp, MX_DOSDIR *dir)
{
	char *filename = dir->dir_name;
	UNUSED(fp);
	if (strcmp(filename, "ppp0") == 0)
	{
		/* hmpf? */
	}
	return 0;
...
static long cdecl pppdev_stat(MX_DOSFD *fp, MAGX_UNSEL *unsel, short rwflag, long /* APPL * */ proc)
{
	struct ppp *ppp = &ppp_priv[0];
	MAGX_UNSEL *s;
