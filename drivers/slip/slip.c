...
#define ESC		((unsigned char)219u)	/* escape char */
#define END		((unsigned char)192u)	/* end of frame char */
#define ESC_ESC	((unsigned char)221u)	/* escaped esc */
#define ESC_END	((unsigned char)220u)	/* escaped end */
...
static long cdecl slip_open(struct netif *);
static long cdecl slip_close(struct netif *);
static long cdecl slip_output(struct netif *, BUF *, const char *, short, short);
static long cdecl slip_ioctl(struct netif *, short, long);
static short slip_send(struct slbuf *);
static short slip_recv(struct slbuf *);

static long slip_init(void);

unsigned char slc_type(BUF *b)
{
	unsigned char c = *b->dstart & 0xf0;
...
long driver_init(void)
{
	...
	return r;
#else
	sockets_dev_init(); /* BUG: return value not checked */
	return slip_init() != 0 ||
		ppp_init() != 0 ? 1 : 0;
#endif
}
...
static long slip_link(struct iflink *ifl)
{
	int i;
	...
	if (slip_priv[i].comp == 0)
	{
		...
		(void) Cconws("slip_link: no space for VJ compression states"); /* BUG: forgotten debug output */
		return ENOMEM;
	}

	slip_priv[i].slbuf = serial_open(&slip_chan[i], ifl->device, slip_send, slip_recv);

	if (slip_priv[i].slbuf == 0)
	{
		DEBUG(("slip_link: no free serial channels"));
		(void) Cconws("slip_link: no free serial channels\r\n"); /* BUG: forgotten debug output */
	...
	sprintf(ifl->ifname, "%s%d", slip_chan[i].name, slip_chan[i].unit);
	return 0;
}

static long cdecl slip_open(struct netif *nif)
{
...
static long cdecl slip_close(struct netif *nif)
{
	struct slip *slp = nif->data;
	long r = 0;
...
static long cdecl slip_output(struct netif *nif, BUF *buf, const char *hwaddr, short hwlen, short pktype)
{
	struct slip *slp = nif->data;
	long r;
	...
	UNUSED(hwlen);
	UNUSED(pktype);
	if ((r = if_enqueue(&nif->snd, buf, buf->info)) != 0)
	{
		slp->stat.o_qfull++;
		nif->out_errors++;
...
static long cdecl slip_ioctl(struct netif *nif, short cmd, long arg)
{
	struct slip *slp = nif->data;
	struct iflink *ifl;
	struct ifreq *ifr;
	...
	case SIOCGIFNAME:
		...
		if (!(slp->flags & SLF_LINKED))
		{
			DEBUG(("slip_ioctl: IFLINK: chan %d not linked", nif->mtu));
			return EINVAL;
		}
	...
	case SIOCGLNKFLAGS:
		...
		ifr->ifr_flags = slp->flags & SLF_USRMASK;
		return 0;

	case SIOCSLNKFLAGS:
		ifr = (struct ifreq *) arg;
		slp->flags &= ~SLF_USRMASK;
		slp->flags |= ifr->ifr_flags & SLF_USRMASK;
		return 0;
...
static long slip_init(void)
{
	...
	sprintf(buf, "SLIP v%s, %d channels\n\r", SLIP_VERSION, SLIP_CHANNELS);
	c_conws(buf);
	for (i = 0; i < SLIP_CHANNELS; ++i)
	{
...
short uncompress(BUF *b, struct slip *slp)
{
	unsigned char type;
	...
	case TYPE_UNCOMPRESSED_TCP:
		...
		if (slp->flags & SLF_AUTOCOMP && slc_uncompress(b, type, slp->comp))
		{
...
static short slip_recv(struct slbuf *slb)
{
...
static short slip_send(struct slbuf *slb)
{
#define SL_PUTC(_c) { buf[head++] = _c; head &= mask; }
	...
	while (space > 0)
	{
		...
		if (slp->snd_buf == 0)
		{
			...
			if (slp->flags & SLF_COMPRESS && IP_PROTO(b) == IPPROTO_TCP)
				*b->dstart |= slc_compress(b, slp->comp, slp->flags & SLF_COMPCID);
