...
static uchar *pool = 0;
static ulong poolsize = POOLSIZE_DEFAULT;
 ...
 */
static void
join_next (register chunk_header *H, ulong state)
{
...
gs_mem_free (void *mem)
{
	...
	if (H != arena)
	{
		...
		if (!H2)
		{
			...
			DEBUG (("gs_mem_free: cannot find block in arena"));
		}
		else if (is_freed (H2))
		{
			join_next (H2, FREED);
...
gs_mem_getfree (int16 flag)
{
	...
	if (flag)
	{
		for (H = arena; H; H = H->next)
		{
			...
			{
				DEBUG (("gs_mem_getfree: possible arena corruption at %p", H));
			}
		}
	}
	else
