...
/*
 * Utitity functions from tcputil.h, may be inlined
 */
#define _tcp_finished(tcb) ((tcb->flags & TCBF_FIN) && SEQLT(tcb->seq_fin, tcb->rcv_nxt))
short tcp_finished(struct tcb *tcb)
{
	return _tcp_finished(tcb);
}
...
{
	return (long) buf->dend - (long) tcph - (tcph->f.b.hdrlen * 4); /* TCP_HDRLEN */
}
...
static void tcbs_closed(struct tcb *tcb, BUF *buf)
{
	UNUSED(tcb);
	tcp_sndrst(buf);
...
static void tcbs_listen(struct tcb *tcb, BUF *buf)
{
	...
	if ((tcph->f.b.flags & (TCPF_RST | TCPF_SYN)) != TCPF_SYN)
	{
		buf_deref(buf, BUF_NORMAL);
		return;
	}

	if (tcph->f.b.flags & TCPF_ACK)
	{
		tcp_sndrst(buf);
		buf_deref(buf, BUF_NORMAL);
		return;
	...
	if (check_syn(tcph->seq, IP_SADDR(buf), IP_DADDR(buf), tcph->srcport, tcph->dstport))
	{
	...
	ntcb->snd_isn =
		ntcb->snd_una = ntcb->snd_wndack = ntcb->snd_nxt = ntcb->snd_max = ntcb->snd_urg = ntcb->seq_write = tcp_isn();

	ntcb->snd_wnd = TCP_MSS;			/* enough for SYN,FIN */

	ntcb->rcv_isn = ntcb->rcv_nxt = ntcb->rcv_wnd = ntcb->rcv_urg = tcph->seq;
	ntcb->seq_read = ntcb->seq_uread = tcph->seq + 1;	/* SEQ of next byte to read() */
	...
	ntcb->snd_mss = ntcb->snd_cwnd = tcp_mss(ntcb, data->dst.addr, r);
...
static void tcbs_synsent(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
	long r;

	if ((tcph->f.b.flags & TCPF_ACK) && (SEQLE(tcph->ack, tcb->snd_isn) || SEQGT(tcph->ack, tcb->snd_max)))
	{
		tcp_sndrst(buf);
		buf_deref(buf, BUF_NORMAL);
		return;
	...
	tcb->rcv_isn = tcb->rcv_nxt = tcb->rcv_wnd = tcb->rcv_urg = tcph->seq;
	tcb->seq_read = tcb->seq_uread = tcph->seq + 1;	/* SEQ of next byte to read() */
	...
	if (SEQGT(tcb->snd_una, tcb->snd_isn))
	{
		...
		KAYDEBUG(("tcp port %d: SYNSENT -> ESTABLISHED", tcb->data->src.port));
	...
	} else
	{
		...
		KAYDEBUG(("tcp port %d: SYNSENT -> SYNRCVD", tcb->data->src.port));

		tcb->state = TCBS_SYNRCVD;
	}

	r = tcp_options(tcb, tcph);
	tcb->snd_mss = tcb->snd_cwnd = tcp_mss(tcb, tcb->data->dst.addr, r);
...
static void tcbs_synrcvd(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
	...
	KAYDEBUG(("tcp port %d: SYNRCVD -> ESTABLISHED", tcb->data->src.port));
	...
	if (tcp_finished(tcb))
	{
		KAYDEBUG(("tcp port %d: ESTABLISHED -> CLOSEWAIT", tcb->data->src.port));
...
static void tcbs_established(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
...
static void tcbs_finwait1(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);

	if (tcph->f.b.flags & (TCPF_RST | TCPF_SYN) || (!tcb->data->sock && tcp_datalen(buf, tcph)))
	{
		tcp_sndrst(buf);
		tcb_reset(tcb, ECONNRESET);
		buf_deref(buf, BUF_NORMAL);
	...
	if (tcp_finished(tcb))
	{
		if (SEQGT(tcb->seq_write, tcb->snd_una))
		{
			...
			KAYDEBUG(("tcp port %d: FINWAIT1 -> CLOSING", tcb->data->src.port));

			tcb->state = TCBS_CLOSING;
		} else
		{
			KAYDEBUG(("tcp port %d: FINWAIT1 -> TIMEWAIT", tcb->data->src.port));
	...
	} else if (SEQLE(tcb->seq_write, tcb->snd_una))
	{
		...
		KAYDEBUG(("tcp port %d: FINWAIT1 -> FINWAIT2", tcb->data->src.port));
...
static void tcbs_finwait2(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
	...
	if (tcp_finished(tcb))
	{
		KAYDEBUG(("tcp port %d: FINWAIT2 -> TIMEWAIT", tcb->data->src.port));
...
static void tcbs_closewait(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
...
static void tcbs_lastack(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
	...
	if (SEQLE(tcb->seq_write, tcb->snd_una))
	{
		 ...
		 * FIN is acked
		 */
		KAYDEBUG(("tcp port %d: LASTACK -> CLOSED", tcb->data->src.port));
...
static void tcbs_closing(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
	...
	{
		/* FIN is acked */
		KAYDEBUG(("tcp port %d: CLOSING -> TIMEWAIT", tcb->data->src.port));
 ...
 */
static void tcbs_timewait(struct tcb *tcb, BUF *buf)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
...
static short check_syn(long syn, long saddr, long daddr, short sport, short dport)
{
	struct in_data *d = tcp_proto.datas;
	struct tcb *tcb;

	while ((d = in_data_lookup_next(d, saddr, sport, daddr, dport, 0)) != NULL)
	{
		tcb = d->pcb;
 ...
 */
#ifdef NOTYET
static void tcp_delack(struct tcb *tcb)
{
#ifdef USE_DELAYED_ACKS
 ...
 */
static long tcp_rcvdata(struct tcb *tcb, BUF *buf)
{
	...
	flags = tcph->f.f.flags;

	/*
	 * Store the segments data length in urgptr
	 */
	datalen = (long) buf->dend - (long) tcph - (tcph->f.f.hdr >> 4) * 4; /* TCP_HDRLEN(tcph) */
	tcph->urgptr = datalen;
#if 0
	DEBUG(("tcp_rcvdata::"));
	tcp_dump(buf);
 ...
 */
static long tcp_addseg(struct in_dataq *q, BUF *buf)
{
	...
	} else
	{
		...
		for (; b->prev; b = b->prev)
		{
			if (SEQLT(SEQNXT(b->prev), SEQ1ST(b)) && SEQLT(seq1st, SEQ1ST(b)) && SEQLT(SEQNXT(b->prev), seqnxt))
			{
				buf->prev = b->prev;
				buf->next = b;
				buf->prev->next = buf;
	 ...
	 * here.
	 */
	while ((b = buf->next) != NULL && SEQLE(SEQNXT(b), seqnxt))
	{
		if (q->qlast == b)
			q->qlast = buf;
		buf->next = b->next;
	...
	while ((b = buf->prev) != NULL && SEQLE(seq1st, SEQ1ST(b)))
	{
		if (q->qfirst == b)
			q->qfirst = buf;
		buf->prev = b->prev;
...
static short tcp_sndwnd(struct tcb *tcb, struct tcp_dgram *tcph)
{
	...
	{
		DEBUG(("tcp_sndwnd: window has been shrunk by %ld bytes", owlast - wlast));
	...
	return (SEQLE(owlast, wlast) && tcb->snd_wnd > 0) ? TCBOE_WNDOPEN : TCBOE_WNDCLOSE;
}

/*
 * Handle incoming acknowledgments
 */
static long tcp_ack(struct tcb *tcb, BUF *buf, short update_sndwnd)
{
	struct tcp_dgram *tcph = (struct tcp_dgram *) IP_DATA(buf);
	short cmd = -1;
	long osnd_wnd;
	...
	if (SEQGT(tcph->ack, tcb->snd_max))
	{
		 ...
		 */
		DEBUG(("tcp_ack(%d): state %d: ack for data not yet sent", tcb->data->src.port, tcb->state));
		KAYDEBUG(("tcp_ack: sndmax %ld, ack %ld", tcb->snd_max, tcph->ack));
	...
	} else if (SEQEQ(tcb->snd_una, tcph->ack) && osnd_wnd == tcph->window && tcp_seglen(buf, tcph) == 0)
	{
		/*
...
long tcp_rcvurg(struct tcb *tcb, BUF *buf)
{
	...
	*tcph;
	...
	hdrlen = (tcph->f.f.hdr >> 4) * 4; /* TCP_HDRLEN(tcph); */
  ...
  adjust:
  	/* XXX TCP_DATA(tcph) */
	memcpy(((char *)(tcph) + (tcph->f.b.hdrlen * 4)), ((char *)(tcph) + (tcph->f.b.hdrlen * 4)) + urglen, datalen - urglen);
	tcph->f.b.flags &= ~TCPF_URG;
	tcph->seq += urglen;
	buf->dend -= urglen;
