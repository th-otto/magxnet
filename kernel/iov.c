...
long iov2buf_cpy(char *buf, long nbytes, const struct iovec *iov, short niov, long skip)
{
	long cando;
	long todo = nbytes;
	...
	{
		cando = MIN(-skip, todo);
		memcpy(buf, (const char *)iov[-1].iov_base + (skip + iov[-1].iov_len), cando);
		buf += cando;
		todo -= cando;
	}
	...
	{
		cando = MIN(todo, (long)iov->iov_len);
...
long buf2iov_cpy(char *buf, long nbytes, const struct iovec *iov, short niov, long skip)
{
	long cando;
	long todo = nbytes;
	...
	{
		cando = MIN(-skip, todo);
		memcpy((char *)iov[-1].iov_base + (skip + iov[-1].iov_len), buf, cando);
		buf += cando;
		todo -= cando;
	}
