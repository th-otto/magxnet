...
void in_proto_register(short protonum, struct in_proto *proto)
{
	UNUSED(protonum);
...
struct in_proto *in_proto_lookup(short protocol)
{
	struct in_proto *p;
...
long in_data_destroy(struct in_data *data, short wait)
{
	short proto;
...
void in_data_flush(struct in_data *data)
{
	BUF *buf;
	BUF *nextbuf;
...
void in_data_put(struct in_data *data)
{
	data->next = data->proto->datas;
	data->proto->datas = data;
}

void in_data_remove(struct in_data *data)
{
...
short in_data_find(short proto, struct in_data *data)
{
...
struct in_data *in_data_lookup(struct in_data *datas, in_addr_t srcaddr, in_port_t srcport, in_addr_t dstaddr, in_port_t dstport)
{
	...
	struct in_data *dead;
	struct in_data *d = datas;

	for (dead = deflt = 0; d; d = d->next)
	{
		if (d->flags & IN_ISBOUND && d->src.port == dstport && ip_same_addr(d->src.addr, dstaddr))
		{
			if (d->flags & IN_ISCONNECTED)
			{
				if (d->dst.port == srcport && ip_same_addr(srcaddr, d->dst.addr))
				{
					if (!(d->flags & IN_DEAD))
						break;
					dead = d;
...
void sa_copy(struct sockaddr *sa1, struct sockaddr *sa2)
{
	short todo;
