...
static long tcp_abort(struct in_data *data, short ostate)
{
...
static long tcp_detach(struct in_data *data, short wait)
{
...
static long tcp_connect(struct in_data *data, const struct sockaddr_in *addr, short addrlen, short nonblock)
{
	...
	data->src.addr = laddr;
	data2 = in_data_lookup(data->proto->datas, data->src.addr, data->src.port, faddr, addr->sin_port);
	...
	tcb->snd_isn =
		tcb->snd_una = tcb->snd_wndack = tcb->snd_nxt = tcb->snd_max = tcb->snd_urg = tcb->seq_write = tcp_isn();
...
static long tcp_listen(struct in_data *data)
{
	struct tcb *tcb = data->pcb;
...
static long tcp_accept(struct in_data *data, struct in_data *newdata, short nonblock)
{
	struct socket *so;
	struct socket **prev;
	struct socket *newso;
...
static long tcp_ioctl(struct in_data *data, short cmd, void *buf)
{
	struct tcb *tcb = data->pcb;
	...
	{
	case SIOCATMARK:
		*(long *) buf = tcp_canreadurg(data, &urgseq) && SEQLE(urgseq, tcb->seq_read);
		return 0;
	...
	case FIOEXCEPT:
		if (data->err || tcp_canreadurg(data, &urgseq) || SEQGT(tcb->rcv_urg, tcb->rcv_nxt))
			space = 1;
		else
			space = 0;
		*(long *) buf = space;
...
static long tcp_select(struct in_data *data, short mode, long proc)
{
	struct socket *so = data->sock;
	struct tcb *tcb = data->pcb;
	long urgseq;
	...
	case O_RDONLY:
		...
		if (tcb->state == TCBS_LISTEN)
			return tcp_canaccept(data) ? 1 : so_rselect(so, proc);
	 ...
	 short flags, const struct sockaddr_in *addr, short addrlen)
{
	...
	while (offset < size)
	{
		...
		if (r < 0)
		{
			...
			return SEQLT(seq_write, tcb->seq_write) ? (tcb->seq_write - seq_write) : r;
		}
...
static void tcp_dropsegs(struct tcb *tcb)
{
	 ...
	 short flags, struct sockaddr_in *addr, short *addrlen)
{
	...
	for (copied = offset = 0; b && size > 0; b = b->next)
	{
		...
		if (oobinline && urgflag && !(flags & MSG_OOB) &&
			SEQLT(readnxt, tcb->seq_uread) && SEQLT(tcb->seq_uread, SEQNXT(b)))
...
static long tcp_shutdown(struct in_data *data, short how)
{
	struct tcb *tcb = data->pcb;
...
static long tcp_input(struct netif *iface, BUF *buf, ulong saddr, ulong daddr)
{
	...
	data = in_data_lookup(tcp_proto.datas, saddr, tcph->srcport, daddr, tcph->dstport);
	if (!data)
	{
		DEBUG(("tcp_input: destination port %d doesn't exist", tcph->dstport));
		tcp_sndrst(buf);
		return -1;
	}
...
static long tcp_error(short type, short code, BUF *buf, in_addr_t saddr, in_addr_t daddr)
{
	...
	data = in_data_lookup(tcp_proto.datas, daddr, tcph->dstport, saddr, tcph->srcport);
	if (!data)
	{
 ...
 */
static long tcp_canreadurg(struct in_data *data, long *urgseq)
{
	BUF *b;
