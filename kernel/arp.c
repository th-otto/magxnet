 ...
 */
static void arp_remove(struct arp_entry *are)
{
...
static void rarp_remove(struct arp_entry *are)
{
	struct arp_entry *curr;
	struct arp_entry **prev;
	short idx;
 ...
 */
#ifdef notused
static void arp_put(struct arp_entry *are)
{
	short idx;
...
static void rarp_put(struct arp_entry *are)
{
	short idx;
...
void arp_free(struct arp_entry *are)
{
 ...
 */
static char *arp_myaddr(struct netif *nif, short type)
{
	switch (type)
	{
	case ARPRTYPE_IP:
...
static void arp_dosend(struct netif *nif, BUF *buf, short pktype)
{
 ...
 */
static void arp_sendreq(struct arp_entry *are)
{
 ...
 */
struct arp_entry *arp_lookup(short flags, struct netif *nif, short type, short len, char *addr)
{
	struct arp_entry *are;
	short idx;
 ...
 */
struct arp_entry *rarp_lookup(short flags, struct netif *nif, short type, short len, char *addr)
{
	struct arp_entry *are;
	short idx;
	...
	for (are = rarptab[idx]; are; are = are->hwnext)
	{
		if ((are->flags & ATF_HWCOM) &&
			...
			(!nif || are->nif == nif) &&
			memcmp(addr, are->hwaddr.adr.bytes, len) == 0)
		{
			are->links++;
			break;
		}
 ...
 */
static void arp_sendq(struct arp_entry *are)
{
	BUF *b;
	 ...
	 */
	while ((b = if_dequeue(&are->outq)) != NULL)
	{
		(*are->nif->output) (are->nif, b, (char *) are->hwaddr.adr.bytes, are->hwaddr.len, PKTYPE_IP);
	}
}
 ...
 */
void arp_input(struct netif *nif, BUF *buf)
{
	char *myaddr;
	char *spr;
	char *dpr;
	 ...
	 * one if not found.
	 */
	are = arp_lookup(forme ? ARLK_NORESOLV : ARLK_NOCREAT, nif, arphdr->prtype, arphdr->prlen, spr);
	if (are)
	{
		/*
	...
	if (arphdr->op == AROP_ARPREQ)
	{
		...
		} else
		{
			 ...
			 */
			are = arp_lookup(ARLK_NOCREAT, 0, arphdr->prtype, arphdr->prlen, dpr);
 ...
 */
void rarp_input(struct netif *nif, BUF *buf)
{
	struct arp_dgram *arphdr;
	struct arp_entry *are;
	char *myaddr;
	 ...
	 */
	are = rarp_lookup(0, nif, arphdr->hwtype, arphdr->hwlen, (char *) ARP_SHW(arphdr));
	if (are && (are->flags & (ATF_PRCOM | ATF_NORARP)) == ATF_PRCOM && are->prtype == arphdr->prtype)
	{
		/*
 ...
 */
static long get_praddr(struct hwaddr *hwaddr, struct sockaddr *sockaddr, short *type)
{
...
#ifdef notused
static long put_praddr(struct hwaddr *hwaddr, struct sockaddr *sockaddr, short type)
{
	switch (type)
	{
	case ARPRTYPE_IP:
...
static long get_hwaddr(struct hwaddr *hwaddr, struct sockaddr *sockaddr, short *type)
{
	struct sockaddr_hw *shw = (struct sockaddr_hw *) sockaddr;

	if (shw->shw_family == AF_LINK && shw->shw_len <= sizeof(hwaddr->adr.bytes))
	{
...
static long put_hwaddr(struct hwaddr *hwaddr, struct sockaddr *sockaddr, short type)
{
	struct sockaddr_hw *shw = (struct sockaddr_hw *) sockaddr;
 ...
 */
#ifdef notused
void arp_dump(struct arp_entry *are)
{
	unsigned char *cp;
	...
	{
		cp = are->praddr.addr;
		DEBUG(("praddr = %x:%x:%x:%x:%x:%x", cp[0], cp[1], cp[2], cp[3], cp[4], cp[5]));
	}

	if (are->flags & ATF_HWCOM)
	{
		cp = are->hwaddr.addr;
		DEBUG(("hwaddr = %x:%x:%x:%x:%x:%x", cp[0], cp[1], cp[2], cp[3], cp[4], cp[5]));
	}
}
#endif
