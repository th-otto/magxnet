...
BUF *masq_ip_input(struct netif *nif, BUF *buf)
{
	...
	} else if (masq.addr == (iph->saddr & masq.mask))
	{
		...
		if (iph->proto == IPPROTO_TCP)
		{
			...
			for (i = 0; tcp_data_modifier[i].data_modify_fn; i++)
			{
				if (tcp_data_modifier[i].port == dst_port)
				{
					...
					{
						/* update info */
						orig_iph = iph = (struct ip_dgram *) buf->dstart;
		...
		if (!db_record)
		{
			...
			case IPPROTO_TCP:
				db_record->timeout = masq.tcp_first_timeout;
				db_record->seq = tcph->seq;
				break;
			case IPPROTO_UDP:
				db_record->timeout = masq.udp_timeout;
				break;
			case IPPROTO_ICMP:
				db_record->timeout = masq.icmp_timeout;
				break;
			}
			db_record->offs = 0;
...
PORT_DB_RECORD *find_port_record(ulong addr, ushort src_port, ushort dst_port, unsigned char proto)
{
	...
	for (i = 0; i < MASQ_NUM_PORTS; i++)
		if (addr == masq.port_db[i]->masq_addr &&
			...
			(proto != IPPROTO_TCP ||
			 !dst_port || dst_port == masq.port_db[i]->dst_port) && proto == masq.port_db[i]->proto)
...
{
	if ((short)record->num >= 0 && record->num < MASQ_NUM_PORTS)
