...
#ifdef DEV_DEBUG
static const char *const ostate_names[] = {
	"IDLE",
	"RETRANS",
	"PERSIST",
	"XMIT"
...
static void tcbos_idle(struct tcb *tcb, short event)
{
	long tmout;
	...
	case TCBOE_SEND:
		{
			...
			tcp_sndhead(tcb);
			tcb->persist_tmo = tcb->retrans_tmo = tmout = tcp_timeout(tcb);
...
static void tcbos_persist(struct tcb *tcb, short event)
{
	switch (event)
	{
	case TCBOE_TIMEOUT:
 ...
 */
static BUF *tcp_mkseg(struct tcb *tcb, ulong size)
{
 ...
 */
static long tcp_sndseg(struct tcb *tcb, BUF *b, short nretrans, long wnd1st, long wndnxt)
{
	struct tcp_dgram *tcph;
	struct tcp_dgram *tcph2;
	long seq1st;
	long seqnxt = 0;
	long offs;
	ulong todo;
	...
	DEBUG(("tcp_sndseg: cut=%x (%ld,%ld,%ld) seq %ld",
		   ...
		   (long) (nb->dend - nb->dstart) - TCP_HDRLEN(tcph), MAX(seqnxt - wndnxt, 0L), seq1st));
#endif
	 ...
	 */
	if (nretrans > 0 && !(cut & TCPF_FIN) && (b2 = b->next) != NULL
 ...
 */
static void tcp_rtt(struct tcb *tcb, BUF *buf)
{
	long err;
...
static void tcp_artt(struct tcb *tcb)
{
	long err;
 ...
 */
static long tcp_dropdata(struct tcb *tcb)
{
	...
	if (n > 0)
	{
		...
		tcb->snd_cwnd += n * ((tcb->snd_cwnd < tcb->snd_thresh)
							  ? tcb->snd_mss : ((tcb->snd_mss * tcb->snd_mss) / tcb->snd_cwnd));
	}
 ...
 */
static long tcp_sndhead(struct tcb *tcb)
{
	long wndnxt;
	long seqnxt;
	long rttseq = 0;
	 ...
	 */
	if (!(tcb->flags & TCBF_NDELAY) && b->info > DATLEN(b) && (SEQLT(tcb->snd_una, tcb->snd_nxt) || q->qfirst != b))
 ...
 */
static short tcp_probe(struct tcb *tcb)
{
	struct tcp_dgram *tcph;
	BUF *b;
 ...
 */
static short tcp_retrans(struct tcb *tcb)
{
	long r;
	BUF *b;
 ...
 */
static short canretrans(struct tcb *tcb)
{
	BUF *b;

	return tcb->snd_wnd > 0 &&
		   (b = tcb->data->snd.qfirst) != NULL &&
...
long tcp_output(struct tcb *tcb, const struct iovec *iov, short niov, long len, long offset, short flags)
{
	...
	if (len == 0 && (flags & (TCPF_SYN | TCPF_FIN)) == 0)
	{
		 ...
		 */
		tcb->flags &= ~TCBF_DELACK;
		return ip_send(tcb->data->src.addr, tcb->data->dst.addr, b, IPPROTO_TCP, 0, &tcb->data->opts);
	}
...
static long tcp_atimeout(struct tcb *tcb)
{
	long tmout;
 ...
 */
long tcp_rcvwnd(struct tcb *tcb, short wnd_update)
{
	long space;
	long minwnd;
