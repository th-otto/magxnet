...
long icmp_errno(short type, short code)
{
...
short icmp_dontsend(short type, BUF *buf)
{
 ...
 */
static BUF *do_send(short type, short code, BUF *buf1, BUF *buf2)
{
	BUF *nbuf;
	struct icmp_dgram *icmph;
 ...
 */
static long do_unreach(BUF *buf, struct netif *nif, long len)
{
	struct icmp_dgram *icmph;
	struct ip_dgram *iph;
 ...
 */
static long do_redir(BUF *b, struct netif *nif, long len)
{
	struct icmp_dgram *icmph;
	struct ip_dgram *iph;
	struct route *rt;
	...
	case ICMPC_NETRD:
		...
		DEBUG(("do_redir: net 0x%lx over gw 0x%lx", iph->daddr & mask, icmph->u.redir_gw));
		route_add(nif, iph->daddr, mask, icmph->u.redir_gw, RTF_DYNAMIC | RTF_MODIFIED | RTF_GATEWAY, RT_TTL, 1);
		break;

	case ICMPC_HOSTRD:
		DEBUG(("do_redir: host 0x%lx over gw 0x%lx", iph->daddr, icmph->u.redir_gw));
		route_add(nif, iph->daddr, 0xffffffffL, icmph->u.redir_gw,
				RTF_DYNAMIC | RTF_MODIFIED | RTF_HOST | RTF_GATEWAY, RT_TTL, 1);
		break;
	}
 ...
 */
static long do_time(BUF *b, struct netif *nif, long len)
{
	struct icmp_dgram *icmph;
	long tm;
	int32_t *longs;
	...
	/*
	 * Time in ms since midnight UTC. What if the
	 * system clock is not set to UTC? Perhaps we should
 ...
 */
static long do_echo(BUF *b, struct netif *nif, long len)
{
	UNUSED(nif);
 ...
 */
static long do_mask(BUF *b, struct netif *nif, long len)
{
	struct icmp_dgram *icmph;
 ...
 */
static long do_info(BUF *b, struct netif *nif, long len)
{
	UNUSED(nif);
	if ((ulong) len < sizeof(struct icmp_dgram))
	{
 ...
 */
static long do_erreport(BUF *b, struct netif *nif, long len)
{
	struct in_ip_ops *p;
	struct icmp_dgram *icmph;
	struct ip_dgram *iph;
	...
	for (p = allipprotos; p; p = p->next)
	{
		...
		{
			(*p->error) (icmph->type, icmph->code, buf, iph->saddr, iph->daddr);
			return 0;
		}
	}
