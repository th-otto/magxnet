...
static long udp_abort(struct in_data *data, short ostate)
{
	UNUSED(data);
	UNUSED(ostate);
	return 0;
...
static long udp_detach(struct in_data *data, short wait)
{
	in_data_destroy(data, wait);
	return 0;
}

static long udp_connect(struct in_data *data, const struct sockaddr_in *addr, short addrlen, short nonblock)
{
	UNUSED(addrlen);
	UNUSED(nonblock);
	if (addr->sin_port == 0)
...
static long udp_accept(struct in_data *data, struct in_data *newdata, short nonblock)
{
	UNUSED(data);
	UNUSED(newdata);
	UNUSED(nonblock);
...
static long udp_ioctl(struct in_data *data, short cmd, void *buf)
{
	switch (cmd)
	{
	case FIONREAD:
...
static long udp_select(struct in_data *data, short mode, long proc)
{
	switch (mode)
	{
	case O_WRONLY:
	 ...
	 short flags, const struct sockaddr_in *addr, short addrlen)
{
	...
	buf = buf_alloc(size + sizeof(struct udp_dgram) + UDP_RESERVE, UDP_RESERVE / 2, BUF_NORMAL);
	if (!buf)
	{
...
static long udp_setsockopt(struct in_data *data, short level, short optname, char *optval, long optlen)
{
	/*
...
static long udp_getsockopt(struct in_data *data, short level, short optname, char *optval, long *optlen)
{
	/*
	 * No special UDP socket options
	 */
...
static long udp_input(struct netif *iface, BUF *buf, in_addr_t saddr, in_addr_t daddr)
{
	...
	data = in_data_lookup(udp_proto.datas, saddr, uh->srcport, daddr, uh->dstport);
	if (!data)
	{
		BUF *nbuf;

		DEBUG(("udp_input: Destination port %d non existant", uh->dstport));
...
static long udp_error(short type, short code, BUF *buf, in_addr_t saddr, in_addr_t daddr)
{
	...
	data = in_data_lookup(udp_proto.datas, daddr, uh->dstport, saddr, uh->srcport);
	if (!data || !(data->flags & IN_ISCONNECTED))
	{
		DEBUG(("udp_error: no such local socket (%lx, %x)", saddr, uh->srcport));
		buf_deref(buf, BUF_NORMAL);
		return 0;
	}
...
#endif
		: "=d"(sum)
		: "g"(srcadr), "d"(dstadr), "i"(IPPROTO_UDP), "d"(len), "0"(sum)
#ifdef __mcoldfire__
		: "d0", "d1", "cc"
#else
		: "d0", "cc"
