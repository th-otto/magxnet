...
static void check_events(PROC *proc, long arg2)
{
	...
	UNUSED(arg2);
	update_head();
	while ((ep = allevents) != NULL && ep->delta <= 0)
	{
		arg = ep->arg;
		func = ep->func;
		allevents = ep->next;
...
static void event_insert(struct event *ep, long delta)
{
	struct event **prev;
	struct event *curr;
...
static short event_remove(struct event *ep)
{
	struct event **prev;
	struct event *curr;
...
void event_del(struct event *ep)
{
	if (ep->func)
	{
		event_remove(ep);
 ...
 */
long event_delta(struct event *ep)
{
	struct event *cur;
	long ticks = 0;
 ...
 */
void event_reset(struct event *ep, long delta)
{
	struct event *curr;
	struct event **prev;
	long ticks = 0;
