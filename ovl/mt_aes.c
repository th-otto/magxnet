							...
							EVNT *event, WORD *global_aes)
{
	...
	aes_params.intin[14] = (WORD) ms;					/* Wîrter drehen */
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 60, 3, 0, 2 };

	aes_params.intin[0] = object;
	aes_params.intin[1] = depth;
	aes_params.intin[2] = windowhandle;

	aes_params.addrin[0] = tree;
	aes_params.addrin[1] = clip;

	_aes_trap(&aes_params, aes_control_data, global_aes);
}


void mt_objc_wchange(OBJECT *tree, WORD object, WORD newstate, GRECT *clip, WORD windowhandle, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 61, 3, 0, 2 };

	aes_params.intin[0] = object;
	aes_params.intin[1] = newstate;
	aes_params.intin[2] = windowhandle;
...
WORD mt_graf_wwatchbox(OBJECT *tree, WORD object, WORD instate, WORD outstate, WORD windowhandle, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 62, 4, 1, 1 };

	aes_params.intin[0] = object;
	aes_params.intin[1] = instate;
	aes_params.intin[2] = outstate;
	aes_params.intin[3] = windowhandle;
...
WORD mt_form_wbutton(OBJECT *tree, WORD object, WORD nclicks, WORD *nextob, WORD windowhandle, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 63, 3, 2, 1 };
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 64, 3, 3, 1 };

	aes_params.intin[0] = object;
	aes_params.intin[1] = ichar;
	aes_params.intin[2] = nextob;
	aes_params.intin[3] = windowhandle;
	...
	*ochar = aes_params.intout[2];
	*onextob = aes_params.intout[1];
	return aes_params.intout[0];
}
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 65, 5, 2, 1 };

	aes_params.intin[0] = object;
	aes_params.intin[1] = edchar;
	aes_params.intin[2] = *didx;
	aes_params.intin[3] = kind;
	aes_params.intin[4] = windowhandle;

	aes_params.addrin[0] = tree;

	_aes_trap(&aes_params, aes_control_data, global_aes);

	*didx = aes_params.intout[1];
	return aes_params.intout[0];
}
...
WORD mt_wind_get_int(WORD whdl, WORD subfn, WORD *g1, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 104, 2, 5, 0 };
...
WORD mt_wind_set_int(WORD whdl, WORD subfn, WORD g1, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 105, 4, 1, 0 };
...
WORD mt_wind_set_ptr_int(WORD whdl, WORD subfn, void *s, WORD g3, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 105, 4, 1, 0 };
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 204, 1, 2, 0 };
...
XEDITINFO *mt_edit_create(WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 210, 0, 0, 0 };
...
WORD mt_edit_open(OBJECT *tree, WORD obj, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 211, 1, 1, 1 };
...
void mt_edit_close(OBJECT *tree, WORD obj, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 212, 1, 0, 1 };
...
void mt_edit_delete(XEDITINFO *xi, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 213, 0, 0, 1 };

	aes_params.addrin[0] = xi;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}
...
WORD mt_edit_cursor(OBJECT *tree, WORD obj, WORD whdl,
					WORD show, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 214, 3, 1, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = whdl;
	aes_params.intin[2] = show;
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	return aes_params.intout[0];
}
...
WORD mt_edit_evnt(OBJECT *tree, WORD obj, WORD whdl,
				EVNT *ev, LONG *errc, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 215, 2, 3, 2 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = whdl;
	aes_params.addrin[0] = tree;
	aes_params.addrin[1] = ev;
	*((LONG *) (aes_params.intout+1)) = 0L;	/* fÅr alte SLB */
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*errc = *((LONG *) (aes_params.intout+1));
	return aes_params.intout[0];
}
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 5, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 0;		/* Subcode 0 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*buf = aes_params.addrout[0];
	*buflen = *((LONG *) (aes_params.intout+1));
	*txtlen = *((LONG *) (aes_params.intout+3));
	return aes_params.intout[0];
}


WORD mt_edit_get_format(OBJECT *tree, WORD obj, WORD *tabwidth, WORD *autowrap, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 3, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 1;		/* Subcode 1 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*tabwidth = aes_params.intout[1];
	*autowrap = aes_params.intout[2];
	return aes_params.intout[0];
}

WORD mt_edit_get_color(OBJECT *tree, WORD obj, WORD *tcolour, WORD *bcolour, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 3, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 2;		/* Subcode 2 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*tcolour = aes_params.intout[1];
	*bcolour = aes_params.intout[2];
	return aes_params.intout[0];
}


WORD mt_edit_get_font(OBJECT *tree, WORD obj, WORD *fontID, WORD *fontH, WORD *fontPix, WORD *mono, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 5, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 3;		/* Subcode 3 */
	aes_params.addrin[0] = tree;
	aes_params.intout[4] = FALSE;	/* fÅr alte Bibliotheken */
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*fontID = aes_params.intout[1];
	*fontH = aes_params.intout[2];
	*fontPix = aes_params.intout[4];
	*mono = aes_params.intout[3];
	return aes_params.intout[0];
}


WORD mt_edit_get_cursor(OBJECT *tree, WORD obj, char **cursorpos, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 1, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 4;		/* Subcode 4 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*cursorpos = aes_params.addrout[0];
	return aes_params.intout[0];
}
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 6, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 5;		/* Subcode 5 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*xscroll = aes_params.intout[1];
	*yscroll = *((LONG *) (aes_params.intout+2));
	*cyscroll = aes_params.addrout[0];
	*cx = aes_params.intout[4];
	*cy = aes_params.intout[5];
	*cursorpos = aes_params.addrout[1];
}


WORD mt_edit_get_dirty(OBJECT *tree, WORD obj, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 1, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 7;		/* Subcode 7 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	return aes_params.intout[0];
}
...
void mt_edit_get_sel(OBJECT *tree, WORD obj, char **bsel, char **esel, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 216, 2, 0, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 8;		/* Subcode 8 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*bsel = aes_params.addrout[0];
	*esel = aes_params.addrout[1];
}
	...
	WORD *global_aes)
{
	...
	aes_params.intin[0] = obj;
	aes_params.intin[1] = 9;		/* Subcode 9 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*nlines = *((LONG *) (aes_params.intout+1));
	*yscroll = *((LONG *) (aes_params.intout+3));
	*yvis = aes_params.intout[5];
	*yval = aes_params.intout[6];
	*ncols = aes_params.intout[7];
	*xscroll = aes_params.intout[8];
	*xvis = aes_params.intout[9];
}


void mt_edit_set_buf(OBJECT *tree, WORD obj, char *buf, LONG buflen, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 4, 0, 2 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 0;		/* Subcode 0 */
	*(LONG *) (aes_params.intin+2) = buflen;
	aes_params.addrin[0] = tree;
	aes_params.addrin[1] = buf;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}


void mt_edit_set_format(OBJECT *tree, WORD obj, WORD tabwidth, WORD autowrap, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 4, 0, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 1;		/* Subcode 1 */
	aes_params.intin[2] = tabwidth;
	aes_params.intin[3] = autowrap;
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}


void mt_edit_set_color(OBJECT *tree, WORD obj, WORD tcolour, WORD bcolour, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 4, 0, 1 };
	...
	aes_params.intin[0] = obj;
	aes_params.intin[1] = 2;		/* Subcode 2 */
	aes_params.intin[2] = tcolour;
	aes_params.intin[3] = bcolour;
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}


void mt_edit_set_font(OBJECT *tree, WORD obj, WORD fontID, WORD fontH, WORD fontPix, WORD mono, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 6, 0, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 3;		/* Subcode 3 */
	aes_params.intin[2] = fontID;
	aes_params.intin[3] = fontH;
	aes_params.intin[4] = mono;
	aes_params.intin[5] = fontPix;
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}


void mt_edit_set_cursor(OBJECT *tree, WORD obj, char *cursorpos, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 2, 0, 2 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 4;		/* Subcode 4 */
	aes_params.addrin[0] = tree;
	aes_params.addrin[1] = cursorpos;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}
...
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 7, 0, 3 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 5;		/* Subcode 5 */
	aes_params.intin[2] = xscroll;
	*(LONG *) (aes_params.intin+3) = yscroll;
	aes_params.intin[5] = cx;
	aes_params.intin[6] = cy;
	aes_params.addrin[0] = tree;
	aes_params.addrin[1] = cyscroll;
	aes_params.addrin[2] = cursorpos;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}


WORD mt_edit_resized(OBJECT *tree, WORD obj, WORD *oldrh, WORD *newrh, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 2, 3, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 6;		/* Subcode 6 */
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	*oldrh = aes_params.intout[1];
	*newrh = aes_params.intout[2];
	return aes_params.intout[0];
}


void mt_edit_set_dirty(OBJECT *tree, WORD obj, WORD dirty, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 2, 0, 2 };
	...
	aes_params.intin[0] = obj;
	aes_params.intin[1] = 7;		/* Subcode 7 */
	aes_params.intin[2] = dirty;
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
}


WORD mt_edit_scroll(OBJECT *tree, WORD obj, WORD whdl, LONG yscroll, WORD xscroll, WORD *global_aes)
{
	MT_PARMDATA aes_params;
	static WORD const aes_control_data[4] = { 217, 6, 1, 1 };

	aes_params.intin[0] = obj;
	aes_params.intin[1] = 9;		/* Subcode 9 */
	aes_params.intin[2] = whdl;
	*(LONG *) (aes_params.intin+3) = yscroll;
	aes_params.intin[5] = xscroll;
	aes_params.addrin[0] = tree;
	_aes_trap(&aes_params, aes_control_data, global_aes);
	return aes_params.intout[0];
}
