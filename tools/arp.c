...
#ifdef __MINT__
#define _PATH_DEV_ARP	"u:/dev/arp"
#else
#define _PATH_DEV_ARP	"u:\\dev\\arp"
#endif
...
static struct arp_info *getarpent(void)
{
	int r;
	...
	r = read(arpfd, &arpinfo, (int)sizeof(arpinfo));
	if (r != sizeof(arpinfo))
		return 0;
...
static char *decode_hw(struct sockaddr_hw *shw)
{
	...
	if (shw->shw_family == AF_LINK)
	{
		...
		case ARPHRD_ETHER:
			if (shw->shw_len != 6)
				break;
			cp = shw->shw_addr;
			sprintf(strbuf, "%x:%x:%x:%x:%x:%x", cp[0], cp[1], cp[2], cp[3], cp[4], cp[5]);
			break;
		}
	}
...
static char *decode_pr(struct sockaddr_hw *shw)
{
	static char strbuf[20];
	struct in_addr ina;
	...
	if (shw->shw_family == AF_LINK)
		...
		case ETHERTYPE_IP:
			if (shw->shw_len == 4)
			{
				...
				in_addr_t *p = (in_addr_t *) shw->shw_addr;

				ina.s_addr = *p;
#else
				ina.s_addr = *((in_addr_t *)shw->shw_addr);
#endif
...
static char *decode_tmout(unsigned long tmout)
{
	static char strbuf[20];
...
static char *decode_flags(unsigned short flags)
{
...
static int get_hwaddr(struct sockaddr_hw *shw, char *addr)
{
	static char str[20];
	unsigned int x,
	 i;
