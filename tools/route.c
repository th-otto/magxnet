...
#define _PATH_DEV_ROUTE		"u:/dev/route"
...
static struct in_addr resolve(char *name)
{
	struct hostent *hent;
...
static void add_route(int argc, char *argv[])
{
...
static void del_route(int argc, char *argv[])
{
	struct rtentry rt;
...
static void show_routes(void)
{
	...
	if (fd < 0)
	{
		fprintf(stderr, "cannot open device %s: %s\n", _PATH_DEV_ROUTE, strerror(errno));
		exit(1);
	}

	printf("Destination         Gateway             " "Flags   Ref      Use Metric Iface\n");

	while (1)
	{
		r = (int)read(fd, &rtinfo, (int)sizeof(rtinfo));
		if (r < 0)
		{
			fprintf(stderr, "cannot read next routing table " "entry from %s: %s\n", _PATH_DEV_ROUTE, strerror(errno));
			exit(1);
		...
		snet = strdup(net.s_addr == INADDR_ANY ? "default" : inet_ntoa(net));

		sgway = strdup((flags & RTF_GATEWAY) ? inet_ntoa(gway) : "*");

		printf("%-20s%-20s%-6s%5d%9ld%7ld %-6s\n",
			   snet, sgway, sflags, rtinfo.rt.rt_refcnt, rtinfo.rt.rt_use, rtinfo.rt.rt_metric, rtinfo.nif);
