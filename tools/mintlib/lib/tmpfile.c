...
FILE *tmpfile()
{
	...
	if (((junknam = tmpnam(NULL)) == NULL) || ((junkfil = fopen(junknam, "w+b")) == NULL))
	{
		if (junknam)
			free(junknam);
		return NULL;
 ...
 */
	...
	if ((++numtmps) == 0)
		...
		file_to_delete = (FILE_RECORD *) realloc(file_to_delete, (size_t) ((numtmps + 1) * sizeof(FILE_RECORD)));
