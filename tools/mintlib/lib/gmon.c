...
*highpc;
{
	...
	s_lowpc = lowpc = (void *) (ROUNDDOWN(USL(lowpc), sizeof(CHUNK) << HIST_SHIFT));
#else
	s_lowpc = lowpc;
#endif
	highpc = (void *) (ROUNDUP(USL(highpc), sizeof(CHUNK) << HIST_SHIFT));
	textsize = USL(highpc) - USL(lowpc);

	/* allocate histogram buffer + gm_header buffer */
	monsize = (textsize >> HIST_SHIFT) * sizeof(CHUNK) + sizeof(struct gm_header);
...
unsigned int nfunc;
{
	...
	if ((hist_size == 0) || (hist_size < (((USL(highpc) - USL(lowpc)) >> HIST_SHIFT) * sizeof(CHUNK))))
	{
		return;
	}
...
*callee;
{
	...
	for (last = top = &tos[ti]; top->link != 0; last = top, top = &tos[top->link])
	{
...
void _mcleanup()
{
	...
	if (_write(fd, (void *) (USL(hist_buffer) - sizeof(struct gm_header)),
			   hist_size + sizeof(struct gm_header)) != (hist_size + sizeof(struct gm_header)))
	{
		Cconws(GMON_FILE);
		Cconws(": error writing\r\n");
		close(fd);
		return;
	}
...
int exc;
{
	...
	*prev;
	...
	for (prev = this; this && (this != me); prev = this,
		 this = (xbra_struct *) ((this->next) ? (((char *) (this->next)) - offsetof(xbra_struct, jump)) : 0))
	{
		/* validate the xbra */
