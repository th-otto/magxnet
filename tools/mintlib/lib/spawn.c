...
static int interpret_script __PROTO((int mode, const char *path, const char *, char *const *argv, char *const *envp));

static int interpret_script(mode, path, _path, argv, envp)
int mode;
const char *path;
const char *_path;
char *const *argv;
...
int _spawnve(mode, _path, argv, envp)
int mode;
const char *_path;
char *const *argv;
char *const *envp;
		...
		if (pconv)
		{
			...
			while (*pc)
			{
				...
				if (!strncmp(p, pc, len) && p[len] == '=')
				{
					...
					strncpy(s, p, len);
					_path_unx2dos(p + len, s + len, left - len);
...
/* MiNT supports overlays with Pexec(200,...) */
	...
	if (rval < 0)
	{
		...
		if (rval == -ENOEXEC)
		{
			...
			if ((umode = getenv("UNIXMODE")) != NULL && strchr(umode, 's') != NULL)
			{
				(void) Mfree(env);
