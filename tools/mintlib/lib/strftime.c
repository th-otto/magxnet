...
size_t strftime(str, maxsize, fmt, ts)
char *str;
size_t maxsize;
const char *fmt;
const struct tm *ts;
...
/* this is the most common case, so it saves us some coding to do it here */
		...
		case 'a':
			...
			if (ts->tm_wday < 0 || ts->tm_wday > 6)
				...
				sprintf(buf, "%-.3s", _LC_Day_name[ts->tm_wday]);
			break;
		case 'B':
		case 'b':
#ifdef SYSV_EXT
		case 'h':						/* same as 'b' */
			...
			if (ts->tm_mon < 0 || ts->tm_mon > 11)
				...
				sprintf(buf, "%-.3s", _LC_Mth_name[ts->tm_mon]);
			break;
		case 'c':
...
static int weeknumber(timeptr, firstweekday)
 ...
 */
{
	return (timeptr->tm_yday - timeptr->tm_wday + (firstweekday ? (timeptr->tm_wday ? 8 : 1) : 7)) / 7;
}
