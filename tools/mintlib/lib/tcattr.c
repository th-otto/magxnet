...
struct termios *stp;
{
	...
	stp->c_iflag = (tcflag_t) (((flags & _TF_BRKINT) ? BRKINT : 0) | ((sg.sg_flags & CRMOD) ? ICRNL : 0) | ((sg.sg_flags & (EVENP | ODDP)) ? INPCK : ((sg.sg_flags & RAW) ? 0 : IGNPAR))	/*keep stty happy */
							   | ((sg.sg_flags & TANDEM) ? (IXON | IXOFF) : 0) | ((sg.sg_flags & RAW) ? 0 : IXON));
...
const struct termios *stp;
{
	...
	sg.sg_flags &= ~(CRMOD | TANDEM | RTSCTS | EVENP | ODDP | TOSTOP | NOFLSH | ECHOCTL | RAW | CBREAK | ECHO | XKEY);
	...
	flags = ((stp->c_cflag & CSTOPB) ? _TF_2STOP : _TF_1STOP) |
		((stp->c_cflag & CLOCAL) ? 0 : _TF_CAR) | ((stp->c_iflag & BRKINT) ? _TF_BRKINT : 0) | (stp->c_cflag & CSIZE);
	state = (stp->c_cflag & HUPCL) ? _TS_HPCL : 0;
	sg.sg_flags |= ((stp->c_cflag & PARENB) ? ((stp->c_cflag & PARODD) ? ODDP : EVENP) : 0);
	sg.sg_flags |= ((stp->c_lflag & (TOSTOP | NOFLSH | ECHOCTL | ECHO))
					| ((stp->c_lflag & IEXTEN) ? XKEY : 0)
					| ((stp->c_lflag & ISIG) ? (stp->c_lflag & ICANON ? 0 : CBREAK) : RAW));
