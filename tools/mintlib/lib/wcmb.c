...
size_t mbstowcs(wstr, mbstr, maxlen)
wchar_t *wstr;
const char *mbstr;
size_t maxlen;
{
	...
	while (revcnt && ((*wstr++ = *mbstr++) != 0))
		revcnt--;

	return maxlen - revcnt;
}

int mbtowc(wstr, mbstr, n)
wchar_t *wstr;
const char *mbstr;
size_t n;
{
...
size_t wcstombs(mbstr, wstr, maxlen)
char *mbstr;
const wchar_t *wstr;
size_t maxlen;
{
	...
	if (wstr == NULL)
		wstr = _wnull;					/* Better avoid L"". */

	while (revcnt && ((*mbstr++ = (char) *wstr++) != 0))
		revcnt--;

	return maxlen - revcnt;
}

int wctomb(mbstr, wchar)
char *mbstr;
wchar_t wchar;
{
