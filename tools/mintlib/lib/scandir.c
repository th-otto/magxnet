...
int scandir __PROTO((char *name, struct direct *** list,
					 int (*selector) (struct direct *), int (*sorter) (const void *, const void *)));

int scandir(name, list, selector, sorter)
char *name;
struct direct ***list;
...
int (*sorter) __PROTO( (const void *, const void *));
{
	...
	for (i = 0; (entp = readdir(dirp)) != NULL;)
		if (selector == NULL || (*selector) (entp))
		{
			...
			{
				size <<= 1;
				names = (struct direct **) realloc((char *) names, size * sizeof names[0]);
				if (names == NULL)
				{
					closedir(dirp);
					return -1;
