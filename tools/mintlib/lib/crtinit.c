...
/* freemem the amount of free mem accounting for MINFREE at top */
		...
		{
			if (_stksize < -1L)
			{							/* keep |_stksize|, use heap for mallocs */
				_stksize = -_stksize;
				_heapbase = (void *) ((long) bp + m);
			}
		}
...
#else
	_monstartup((void *) (bp->p_tbase), (void *) ((long) bp->p_tbase + bp->p_tlen));
#endif
...
/* if we find MWC arguments, tie off environment here */
		if (*from == 'A' && *(from + 1) == 'R' && *(from + 2) == 'G' && *(from + 3) == 'V' && *(from + 4) == '=')
		{
			*envp++ = (char *) 0;
			count += 4;
			*from = 0;
			 ...
			 */
			if (*from == 'N' && *(from + 1) == 'U' && *(from + 2) == 'L' && *(from + 3) == 'L' && *(from + 4) == ':')
			{
				null_list = from + 5;
			}
