...
INLINE static void
#if __STDC__
insert_sort(void *base, void *end_ptr, size_t siz, int (*cmp)(const void *, const void *))
#else
...
#endif
{
	...
	for (run_ptr = (void *) ((char *) end_ptr - siz); run_ptr >= base; DEC(run_ptr, siz))
...
INLINE static void *
#if __STDC__
find_pivot(void *low, void *high, size_t siz, int (*cmp)(const void *, const void *))
#else
...
int (*cmp) __PROTO( (const void *, const void *));
{
	if (total_elems > 1)
	{
		...
		do
		{
		  next:if ((char *) hi <= ((char *) lo + Thresh))
														/* correct unsigned comapare */
			{
