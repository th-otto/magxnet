...
#ifndef COPYING_UNIT
#ifdef atarist
#define COPYING_UNIT long				/* a long is ok because of the alignment above */
#else
...
void _obstack_begin(h, size, alignment, chunkfun, freefun)
struct obstack *h;
size_t size;
int alignment;
...
size_t length;
{
	...
	if (h->alignment_mask + 1 >= DEFAULT_ALIGNMENT)
	{
		for (i = obj_size / sizeof(COPYING_UNIT) - 1; i >= 0; i--)
			((COPYING_UNIT *) new_chunk->contents)[i] = ((COPYING_UNIT *) h->object_base)[i];
...
size_t (obstack_object_size) (obstack)
struct obstack *obstack;
{
	return obstack_object_size(obstack);
}

size_t (obstack_room) (obstack)
struct obstack *obstack;
{
