...
char **endptr;
{
	...
	if (Ise(*s))
	{
		...
		{								/* ! ([s]xxx[.[yyy]]e)  */
			...
			if (s == tmp)
				s++;					/* back up pointer for a trailing e|E|d|D */
		}
	}
...
char **endptr;
{
	...
	/* underflow -- in reality occurs before this */
	/* FIXME */
	if (texp < MINE)
	{
		errno = ERANGE;
