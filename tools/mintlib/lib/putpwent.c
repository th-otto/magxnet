...
FILE *f;
{
	if ((p->pw_passwd[0] == 0x00) || (p->pw_age[0] == 0x00))
	{
		...
		if (fprintf(f, "%s:%s:%d:%d:%s:%s:%s\n",
					p->pw_name, p->pw_passwd, p->pw_uid, p->pw_gid, p->pw_gecos, p->pw_dir, p->pw_shell) == EOF)
			return (-1);
		else
			return (fflush(f));
	} else
	{
		...
		if (fprintf(f, "%s:%s,%s:%d:%d:%s:%s:%s\n",
					p->pw_name,
					p->pw_passwd, p->pw_age, p->pw_uid, p->pw_gid, p->pw_gecos, p->pw_dir, p->pw_shell) == EOF)
			return (-1);
		else
			return (fflush(f));
	}
