...
/* BUG: fix devices names for MagiCNet */
#define UNIX_DEVICE	"/dev/unix"
#define INET_DEVICE	"u:\\dev\\inet"
...
static const char *un_decode_state(short state)
{
	switch (state)
	{
	case SS_VIRGIN:
...
static const char *un_decode_type(short type)
{
	switch (type)
	{
	case SOCK_DGRAM:
...
static const char *un_decode_flags(short flags)
{
...
static void show_unix(void)
{
	...
	for (;;)
	{
		r = (int)read(fd, &info, (int)sizeof(info));
		if (r == 0)
			break;
		if (r < 0)
		{
 ...
 * INET domain stuff.
 */
static const char *in_decode_state(short state)
{
	switch (state)
	{
	case TCPS_CLOSED:
...
static char *in_decode_addr(struct sockaddr_in *in, const char *proto)
{
	static char buf[40];
	struct servent *sent;
	...
	} else
	{
		...
		if (sent)
		{
			sprintf(buf, "%s:%s", in->sin_addr.s_addr == INADDR_ANY ? "*" : inet_ntoa(in->sin_addr), sent->s_name);
		} else
		{
			sprintf(buf, "%s:%d", in->sin_addr.s_addr == INADDR_ANY ? "*" : inet_ntoa(in->sin_addr), in->sin_port);
		}
	}
...
static const char *in_decode_proto(short proto)
{
	switch (proto)
	{
	case IPPROTO_UDP:
...
static void show_udp(void)
{
	...
	for (;;)
	{
		...
		if (r < 0)
		{
			fprintf(stderr, "cannot read next entry from %s: %s", INET_DEVICE, strerror(errno));
			break;
		}
		...
		printf("%-6s %6lu %6lu %-20s %-20s %-15s\n",
			   ...
			   info.recvq, info.sendq, laddr, faddr,
